...
export default class SignUpNoRegComponent extends Component {
  ...
  onSubmit = (e) => {
    ...
    if (!username || username.length < 3 || username.length > 12) return this.setState({signUpError: true, errorMsg: 'Invalid username input, make sure its three (3-12) characters or longer'})
    if (!password || password.length < 4 || password.length > 20) return this.setState({signUpError: true, errorMsg: 'Invalid password input, make sure its 4 (four) characters or longer'})
